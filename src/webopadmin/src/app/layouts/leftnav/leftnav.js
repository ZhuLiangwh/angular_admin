define(['common/utils/tree'], function (TreeUtil) {
    ...
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope', '$state', '$location',LeftNavCtrl]);
            return mod;
        }
    };

    function LeftNavCtrl($rootScope, $scope, $state, $location) {
        $scope.navData = null;
        $scope.treeStatus = {
            currentNode: null,
            expandedNodes: []
        ...
        $rootScope.$on('$stateChangeSuccess', function (event, toState, toParams, fromState, fromParams) {
            var state = toState.name.split('.'),
                navData = $scope.navData || ($scope.navData = JSON.parse(window.localStorage['user_session'])['navData']),
                selectedNode;
