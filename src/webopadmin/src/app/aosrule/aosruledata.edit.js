define([], function () {
    ...
    function AosruledataEditCtrl($scope, $state, $stateParams,$q, $window,$location, DS, dpDialog) {
        ...
        $scope.afterVali = function(){
            ...
            if(!$scope.entity.min_version)
            {
                $scope.entity.min_version = 0;
            }
            if(!$scope.entity.max_version)
            {
                $scope.entity.max_version = 4294967295;
            }
            if(!$scope.entity.max_value)
            {
                $scope.entity.max_value = 0;
            }
            if(!$scope.entity.gray_scale)
            {
                $scope.entity.gray_scale = 100;
            }
            if(!$scope.entity.gray_start)
            {
                $scope.entity.gray_start = 1;
            }
        }
