define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'AthemepushhomeListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.athemepushhome', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', AthemepushhomeListCtrl]);
            return mod;
        }
    };

    function AthemepushhomeListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addAthemepushhome = function () {
            $state.go('themerecommended.add-athemepushhome');
        };

        $scope.ViewDetail = function (item) {
            $state.go('themerecommended.edit-athemepushhome', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.athemepushhomeTableParams.page(1);
            $scope.athemepushhomeTableParams.reload();
        };
        ...
        $scope.athemepushhomeTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        },{
