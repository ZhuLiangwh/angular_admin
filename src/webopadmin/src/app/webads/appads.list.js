define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'AppadsListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.appads', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', AppadsListCtrl]);
            return mod;
        }
    };

    function AppadsListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addAppads = function () {
            $state.go('webads.add-appads');
        };

        $scope.ViewDetail = function (item) {
            $state.go('webads.edit-appads', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.applistTableParams.page(1);
            $scope.applistTableParams.reload();
        };
        ...
        $scope.applistTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        },{
