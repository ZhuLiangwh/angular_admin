define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'ApplistListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.applist', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', ApplistListCtrl]);
            return mod;
        }
    };

    function ApplistListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addApplist = function () {
            $state.go('webads.add-applist');
        };

        $scope.ViewDetail = function (item) {
            $state.go('webads.edit-applist', {
                id: item.id
            });
        };
