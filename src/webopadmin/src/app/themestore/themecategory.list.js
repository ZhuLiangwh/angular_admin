define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'ThemecategoryListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.themecategory', 'logger', 'apiService', 'PER_PAGE','dpErrorHandler','dpDialog', ThemecategoryListCtrl]);
            return mod;
        }
    };

    function ThemecategoryListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpErrorHandler,dpDialog) {
        ...
        $scope.addThemecategory = function () {
            $state.go('themestore.add-themecategory');
        };

        $scope.ViewDetail = function (item) {
            $state.go('themestore.edit-themecategory', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.themecategoryTableParams.page(1);
            $scope.themecategoryTableParams.reload();
        };
        ...
        $scope.themecategoryTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
