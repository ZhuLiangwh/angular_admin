define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'ThemesubjectListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.themesubject', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', ThemesubjectListCtrl]);
            return mod;
        }
    };

    function ThemesubjectListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addThemesubject = function () {
            $state.go('themestore.add-themesubject');
        };

        $scope.ViewDetail = function (item) {
            $state.go('themestore.edit-themesubject', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.themesubjectTableParams.page(1);
            $scope.themesubjectTableParams.reload();
        };
        ...
        $scope.themesubjectTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
