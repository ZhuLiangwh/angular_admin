define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'AthemelocaleListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.athemelocale', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', AthemelocaleListCtrl]);
            return mod;
        }
    };

    function AthemelocaleListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addAthemelocale = function () {
            $state.go('staticresource.add-athemelocale');
        };

        $scope.ViewDetail = function (item) {
            $state.go('staticresource.edit-athemelocale', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.AthemelocaleTableParams.page(1);
            $scope.AthemelocaleTableParams.reload();
        };
        ...
        $scope.AthemelocaleTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
