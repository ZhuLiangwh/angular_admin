define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AthemeListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter','$location', '$modal', 'ngTableParams', 'ds.atheme', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', AthemeListCtrl]);
            return mod;
        }
    };

    function AthemeListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.addAtheme = function () {
            $state.go('staticresource.add-atheme');
        };

        $scope.ViewDetail = function (item) {
            $state.go('staticresource.edit-atheme',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.athemeTableParams.page(1);
            $scope.athemeTableParams.reload();
        };
        ...
        $scope.athemeTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
