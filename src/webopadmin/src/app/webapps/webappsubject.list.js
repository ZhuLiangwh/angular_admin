define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'WebappsubjectListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.webappsubject', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', WebappsubjectListCtrl]);
            return mod;
        }
    };

    function WebappsubjectListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addWebappsubject = function () {
            $state.go('webapps.add-webappsubject');
        };

        $scope.ViewDetail = function (item) {
            $state.go('webapps.edit-webappsubject', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.webappsubjectTableParams.page(1);
            $scope.webappsubjectTableParams.reload();
        };
        ...
        $scope.webappsubjectTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
