define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'WebappcategoryListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.webappcategory', 'logger', 'apiService', 'PER_PAGE','dpErrorHandler','dpDialog', WebappcategoryListCtrl]);
            return mod;
        }
    };

    function WebappcategoryListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpErrorHandler,dpDialog) {
        ...
        $scope.addWebappcategory = function () {
            $state.go('webapps.add-webappcategory');
        };

        $scope.ViewDetail = function (item) {
            $state.go('webapps.edit-webappcategory', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.webappcategoryTableParams.page(1);
            $scope.webappcategoryTableParams.reload();
        };
        ...
        $scope.webappcategoryTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
