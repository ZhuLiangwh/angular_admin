define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'WebappListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter','cache','$location', '$modal', 'ngTableParams', 'ds.webapp', 'logger', 'apiService','dpErrorHandler','dpDialog', 'PER_PAGE', WebappListCtrl]);
            return mod;
        }
    };

    function WebappListCtrl($scope, $window, $state, $filter,cache, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,dpDialog,PER_PAGE) {
        ...
        $scope.addWebapp = function () {
            $state.go('webapps.add-webapp');
        };

        $scope.ViewDetail = function (item) {
            $state.go('webapps.edit-webapp',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.webappTableParams.page(1);
            $scope.webappTableParams.reload();
        };

        $scope.filter_platform = function (node) {
            var selectedValue = node.selectedValue;
            _.extend(apiParams, selectedValue);
            $scope.webappTableParams.page(1);
            $scope.webappTableParams.reload();
        ...
        $scope.webappTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
