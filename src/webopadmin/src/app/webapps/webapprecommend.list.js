define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'WebapprecommendListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.webapprecommend', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', WebapprecommendListCtrl]);
            return mod;
        }
    };

    function WebapprecommendListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addWebapprecommend = function () {
            $state.go('webapps.add-webapprecommend');
        };

        $scope.ViewDetail = function (item) {
            $state.go('webapps.edit-webapprecommend', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.webapprecommendTableParams.page(1);
            $scope.webapprecommendTableParams.reload();
        };
        ...
        $scope.webapprecommendTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
