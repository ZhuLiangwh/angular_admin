define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'AthemefeaturedListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.athemefeatured', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', AthemefeaturedListCtrl]);
            return mod;
        }
    };

    function AthemefeaturedListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addAthemefeatured = function () {
            $state.go('newthemestore.add-athemefeatured');
        };

        $scope.ViewDetail = function (item) {
            $state.go('newthemestore.edit-athemefeatured', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.athemefeaturedTableParams.page(1);
            $scope.athemefeaturedTableParams.reload();
        };
        ...
        $scope.athemefeaturedTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
