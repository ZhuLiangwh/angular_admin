define([], function () {
    var diName = 'AthemepushstoreEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state', '$window', '$location', 'ds.athemepushstore', '$log','dpDialog', AthemepushstoreEditCtrl]);
            return mod;
        }
    };

    function AthemepushstoreEditCtrl($rootScope, $scope,apiService, $state, $window, $location, DS, $log,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/athemepushstore/check_unique');
        }
        ...
        $scope.popUp = function(method, moduleName, modelName,label,index,params,single){
            ...
            dpDialog.loadIframe({
                ...
                from:'newthemestore_athemepushstore'
            });
        };
