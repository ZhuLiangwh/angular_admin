define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'AthemecategoryListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.athemecategory', 'logger', 'apiService', 'PER_PAGE','dpErrorHandler','dpDialog', AthemecategoryListCtrl]);
            return mod;
        }
    };

    function AthemecategoryListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpErrorHandler,dpDialog) {
        ...
        $scope.addAthemecategory = function () {
            $state.go('newthemestore.add-athemecategory');
        };

        $scope.ViewDetail = function (item) {
            $state.go('newthemestore.edit-athemecategory', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.athemecategoryTableParams.page(1);
            $scope.athemecategoryTableParams.reload();
        };
        ...
        $scope.athemecategoryTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
