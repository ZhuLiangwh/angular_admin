define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'AthemepushstoreListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.athemepushstore', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', AthemepushstoreListCtrl]);
            return mod;
        }
    };

    function AthemepushstoreListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addAthemepushstore = function () {
            $state.go('newthemestore.add-athemepushstore');
        };

        $scope.ViewDetail = function (item) {
            $state.go('newthemestore.edit-athemepushstore', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.athemepushstoreTableParams.page(1);
            $scope.athemepushstoreTableParams.reload();
        };
        ...
        $scope.athemepushstoreTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        },{
