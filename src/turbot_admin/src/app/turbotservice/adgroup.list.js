define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AdgroupListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.adgroup', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', AdgroupListCtrl]);
            return mod;
        }
    };

    function AdgroupListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.addAdgroup = function () {
            $state.go('turbotservice.add-adgroup');
        };

        $scope.ViewDetail = function (item) {
            $state.go('turbotservice.edit-adgroup',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.adgroupTableParams.page(1);
            $scope.adgroupTableParams.reload();
        };
        ...
        $scope.adgroupTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
