define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AdpushListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.adpush', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', AdpushListCtrl]);
            return mod;
        }
    };

    function AdpushListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addAdpush = function () {
            $state.go('turbotservice.add-adpush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('turbotservice.edit-adpush',{id:item.id});
        };

        $scope.ViewStatistics = function(item){
            $state.go('turbotservice.adstatistics',{id:item.id});
        }
        ...
        $scope.filter = function (node) {
            ...
            $scope.adpushTableParams.page(1);
            $scope.adpushTableParams.reload();
        };
        ...
        $scope.adpushTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
