define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AdListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.ad', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', AdListCtrl]);
            return mod;
        }
    };

    function AdListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addAd = function () {
            $state.go('turbotservice.add-ad');
        };

        $scope.ViewDetail = function (item) {
            $state.go('turbotservice.edit-ad',{id:item.id});
        };
