define([], function () {
    var diName = 'AdposEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$q','$state', '$window', '$location', 'ds.adpos', '$log', 'dpDialog',AdposEditCtrl]);
            return mod;
        }
    };

    function AdposEditCtrl($rootScope, $scope,apiService,$q, $state, $window, $location, DS, $log, dpDialog) {
        ...
        $scope.autooperation_type = [
            {value:true,name:'是'},
            {value:false,name:'否'}
        ];
        $scope.entity.autooperation = $scope.autooperation_type[0].value;
            ...
            if($scope.displayInfo) {
                ...
                DS.protodata()
                    .then(function(data) {
                        ...
                        $scope.entity.position = $scope.displayInfo.position[0].number;
