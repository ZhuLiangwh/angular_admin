define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AdsourceListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.adsource', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', 'dpDialog', AdsourceListCtrl]);
            return mod;
        }
    };

    function AdsourceListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE, dpDialog) {
        ...
        $scope.addAdsource = function () {
            $state.go('turbotservice.add-adsource');  
        };

        $scope.ViewDetail = function (item) {
            $state.go('turbotservice.edit-adsource',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.adsourceTableParams.page(1);
            $scope.adsourceTableParams.reload();
        };
        ...
        $scope.adsourceTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
