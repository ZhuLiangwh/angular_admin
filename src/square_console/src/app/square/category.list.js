define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl, alertDialogTpl) {
    var diName = 'CategoryListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.category', 'logger', 'apiService', 'dpErrorHandler', 'PER_PAGE','dpDialog', CategoryListCtrl]);
            return mod;
        }
    };

    function CategoryListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService,dpErrorHandler, PER_PAGE,dpDialog) {
        ...
        $scope.addCategory = function () {
            $state.go('square.add-category');
        };

        $scope.ViewDetail = function (item) {
            $state.go('square.edit-category', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.categoryTableParams.page(1);
            $scope.categoryTableParams.reload();
        };
        ...
        $scope.categoryTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
