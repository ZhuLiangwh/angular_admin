define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'HotappListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.hotapp', 'logger', 'apiService', 'dpErrorHandler','PER_PAGE','dpDialog', HotappListCtrl]);
            return mod;
        }
    };

    function HotappListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.addHotapp = function () {
            $state.go('square.add-hotapp');
        };

        $scope.ViewDetail = function (item) {
            $state.go('square.edit-hotapp',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.hotappTableParams.page(1);
            $scope.hotappTableParams.reload();
        };
        ...
        $scope.hotappTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
