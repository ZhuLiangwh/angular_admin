define(['text!layouts/confirm-dialog.html'], function (confirmDialogTpl) {
    ...
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aospackage', 'logger', 'apiService', 'PER_PAGE', AospackageListCtrl]);
            return mod;
        }
    };

    function AospackageListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE) {
        var apiParams = {};
        var vm = $scope.vm = {
            'listChecked': [],
            'listTotal': 0
