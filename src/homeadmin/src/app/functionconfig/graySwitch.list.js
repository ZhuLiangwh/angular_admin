define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'GraySwitchListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.graySwitch', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', GraySwitchListCtrl]);
            return mod;
        }
    };

    function GraySwitchListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addgraySwitch = function () {
            $state.go('functionconfig.add-graySwitch');
        };

        $scope.ViewDetail = function (item) {
            $state.go('functionconfig.edit-graySwitch',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.graySwitchTableParams.page(1);
            $scope.graySwitchTableParams.reload();
        };
        ...
        $scope.graySwitchTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
