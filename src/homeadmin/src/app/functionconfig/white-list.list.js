define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'WhiteListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.whiteList', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', WhiteListCtrl]);
            return mod;
        }
    };

    function WhiteListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addwhiteList = function () {
            $state.go('functionconfig.add-white-list');
        };

        $scope.ViewDetail = function (item) {
            $state.go('functionconfig.edit-white-list',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.whiteListTableParams.page(1);
            $scope.whiteListTableParams.reload();
        };
        ...
        $scope.whiteListTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
