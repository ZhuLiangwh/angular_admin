define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'CardListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.card', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', CardListCtrl]);
            return mod;
        }
    };

    function CardListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addCard = function () {
            $state.go('homepage.add-card');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-card',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.cardTableParams.page(1);
            $scope.cardTableParams.reload();
        };
        ...
        $scope.cardTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
