define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'BannerListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter','cache','$location', '$modal', 'ngTableParams', 'ds.banner', 'logger', 'apiService','dpErrorHandler','dpDialog', 'PER_PAGE', BannerListCtrl]);
            return mod;
        }
    };

    function BannerListCtrl($scope, $window, $state, $filter,cache, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,dpDialog,PER_PAGE) {
        ...
        $scope.addBanner = function () {
            $state.go('homepage.add-banner');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-banner',{id:item.id});
        };
        ...
        $scope.reviewSubmit = function(){
            action('submit', 'submit', function (data) {
                changeStatus(data);
            },function (data) {
                changeStatus(data);
                errorHandler.show(data);
        ...
        $scope.selectCache = cache.get('bannerSession');

        //根据缓存的参数加载数据
        $scope.loadDataFromCache = function(){
            apiParams = $scope.selectCache;
            $scope.transTableParams.page(apiParams.page);
            $scope.transTableParams.reload();
        };
        $scope.filter = function (node, isInit) {
            if (!isInit) {
                apiParams = node.selectedValue;
                $scope.bannerTableParams.page(1);
                $scope.bannerTableParams.reload();
            }
        };
        ...
        $scope.bannerTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
            ...
            getData: function ($defer, params) {
                //设置缓存参数，如过滤条件和当前页数
                $scope.selectCache = _.extend($scope.selectCache,apiParams,{'page':params.page()});
                cache.set({'bannerSession': $scope.selectCache});
