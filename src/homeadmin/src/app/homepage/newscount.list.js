define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'NewscountListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.newscount', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', NewscountListCtrl]);
            return mod;
        }
    };

    function NewscountListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addnewsCount = function () {
            $state.go('homepage.add-newsCount');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-newsCount',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.newscountTableParams.page(1);
            $scope.newscountTableParams.reload();
        };
        ...
        $scope.newscountTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
