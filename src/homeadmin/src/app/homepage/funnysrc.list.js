define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'FunnysrcListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.funnysrc', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', FunnysrcListCtrl]);
            return mod;
        }
    };

    function FunnysrcListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addfunnysrc = function () {
            $state.go('homepage.add-funnysrc');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-funnysrc',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.funnysrcTableParams.page(1);
            $scope.funnysrcTableParams.reload();
        };
        ...
        $scope.funnysrcTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
