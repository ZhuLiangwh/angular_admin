define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'JPNewsCateListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.JPNewsCate', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', JPNewsCateListCtrl]);
            return mod;
        }
    };

    function JPNewsCateListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.addJPNewsCate = function () {
            $state.go('homepage.add-JPNewsCate');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-JPNewsCate',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.JPNewsCateTableParams.page(1);
            $scope.JPNewsCateTableParams.reload();
        };
        ...
        $scope.JPNewsCateTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
