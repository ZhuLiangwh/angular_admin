define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'JPNewsSourceListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.JPNewsSource', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', JPNewsSourceListCtrl]);
            return mod;
        }
    };

    function JPNewsSourceListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addJPNewsSource = function () {
            $state.go('homepage.add-JPNewsSource');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-JPNewsSource',{id:item.id});
        };
        var tips = {
        ...
        $scope.filter = function (node) {
            ...
            $scope.JPNewsSourceTableParams.page(1);
            $scope.JPNewsSourceTableParams.reload();
        };
        ...
        $scope.JPNewsSourceTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
