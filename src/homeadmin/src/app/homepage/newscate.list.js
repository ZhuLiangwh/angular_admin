define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'NewscateListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.newscate', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', NewscateListCtrl]);
            return mod;
        }
    };

    function NewscateListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addnewsCate = function () {
            $state.go('homepage.add-newsCate');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-newsCate',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.newscateTableParams.page(1);
            $scope.newscateTableParams.reload();
        };
        ...
        $scope.newscateTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
