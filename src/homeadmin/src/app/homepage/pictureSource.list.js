define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'PictureSourceListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.pictureSource', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', PictureSourceListCtrl]);
            return mod;
        }
    };

    function PictureSourceListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addpictureSource = function () {
            $state.go('homepage.add-pictureSource');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-pictureSource',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.pictureSourceTableParams.page(1);
            $scope.pictureSourceTableParams.reload();
        };
        ...
        $scope.pictureSourceTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
