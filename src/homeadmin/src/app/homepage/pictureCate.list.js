define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'PictureCateListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.pictureCate', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', PictureCateListCtrl]);
            return mod;
        }
    };

    function PictureCateListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.addPictureCate = function () {
            $state.go('homepage.add-pictureCate');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-pictureCate',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.pictureCateTableParams.page(1);
            $scope.pictureCateTableParams.reload();
        };
        ...
        $scope.pictureCateTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
