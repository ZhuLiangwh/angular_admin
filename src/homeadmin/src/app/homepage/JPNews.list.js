define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'JPNewsListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.JPNews', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', JPNewsListCtrl]);
            return mod;
        }
    };

    function JPNewsListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addJPNews = function () {
            $state.go('homepage.add-JPNews');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-JPNews',{id:item.id});
        };
        var tips = {
        ...
        $scope.filter = function (node) {
            ...
            $scope.JPNewsTableParams.page(1);
            $scope.JPNewsTableParams.reload();
        };
        ...
        $scope.JPNewsTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
