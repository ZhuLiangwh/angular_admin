define([], function () {
    var diName = 'JPNewsEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state','$modal', '$window', '$location', 'ds.JPNews', '$log', 'dpDialog',JPNewsEditCtrl]);
            return mod;
        }
    };

    function JPNewsEditCtrl($rootScope, $scope,apiService, $state,$modal, $window, $location, DS, $log, dpDialog) {
        ...
        if (isEditState) {
            DS.edit({
                ...
                $scope.entity.cate_id = $scope.entity.JPNewsCate.id;
            });
        }else{
            $scope.entity.order = 0;
        }
