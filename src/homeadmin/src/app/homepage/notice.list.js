define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'NoticeListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.notice', 'logger', 'apiService','dpErrorHandler','dpDialog', 'PER_PAGE', NoticeListCtrl]);
            return mod;
        }
    };

    function NoticeListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,dpDialog,PER_PAGE) {
        ...
        $scope.addNotice = function () {
            $state.go('homepage.add-notice');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-notice',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.noticeTableParams.page(1);
            $scope.noticeTableParams.reload();
        };
        ...
        $scope.noticeTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
