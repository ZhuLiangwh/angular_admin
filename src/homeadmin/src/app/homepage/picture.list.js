define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'PictureListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.picture', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', 'dpDialog', PictureListCtrl]);
            return mod;
        }
    };

    function PictureListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.filter = function (node) {
            ...
            $scope.pictureTableParams.page(1);
            $scope.pictureTableParams.reload();
        };
        ...
        $scope.pictureTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
