define([], function () {
    var diName = 'JPNewsSubCateEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state','$modal', '$window', '$location', 'ds.JPNewsSubCate', '$log', 'dpDialog',JPNewsSubCateEditCtrl]);
            return mod;
        }
    };
    function JPNewsSubCateEditCtrl($rootScope, $scope,apiService, $state,$modal, $window, $location, DS, $log, dpDialog) {
        ...
        $scope.afterVali = function(){
            if(!$scope.entity.JPNewsCate.title || $scope.entity.JPNewsCate.length === 0){
                alert('大分类名称不能为空');
                return false;
            }
        }
