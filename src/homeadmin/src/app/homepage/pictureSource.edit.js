define([], function () {
    var diName = 'PictureSourceEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state','$modal', '$window', '$location', 'ds.pictureSource', '$log', 'dpDialog',PictureSourceEditCtrl]);
            return mod;
        }
    };

    function PictureSourceEditCtrl($rootScope, $scope,apiService, $state,$modal, $window, $location, DS, $log, dpDialog) {
        ...
        if (isEditState) {
            DS.edit({
                ...
                $scope.entity.pictureCate={
                    id : $scope.entity.catedata.id,
                    title : $scope.entity.catedata.title
                };
            });
        }
        ...
        $scope.afterVali = function(){
            if( !$scope.entity.pictureCate.title || $scope.entity.pictureCate.length === 0){
                alert('类型不能为空');
                return false;
            }
