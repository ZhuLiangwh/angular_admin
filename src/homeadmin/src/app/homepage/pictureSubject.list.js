define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'PictureSubjectListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.pictureSubject', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', PictureSubjectListCtrl]);
            return mod;
        }
    };

    function PictureSubjectListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addPictureSubject = function () {
            $state.go('homepage.add-pictureSubject');
        };
        ...
        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-pictureSubject',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.pictureSubjectTableParams.page(1);
            $scope.pictureSubjectTableParams.reload();
        };
        ...
        $scope.pictureSubjectTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
