define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AtlasListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', 'cache', '$location', '$modal', 'ngTableParams', 'ds.atlas', 'logger', 'apiService','dpErrorHandler','dpDialog', 'PER_PAGE', AtlasListCtrl]);
            return mod;
        }
    };

    function AtlasListCtrl($scope, $window, $state, $filter, cache, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,dpDialog,PER_PAGE) {
        ...
        $scope.addAtlas = function () {
            $state.go('homepage.add-atlas');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-atlas',{id:item.id});
        };
        ...
        $scope.selectCache = cache.get('atlasSession');
        ...
        $scope.filter = function (node, isInit) {
            if (!isInit) {
                ...
                $scope.atlasTableParams.page(1);
                $scope.atlasTableParams.reload();
            }
        };
        ...
        $scope.atlasTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
