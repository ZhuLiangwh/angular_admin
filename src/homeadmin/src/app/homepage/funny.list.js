define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'FunnyListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.funny', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', FunnyListCtrl]);
            return mod;
        }
    };

    function FunnyListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addfunny = function () {
            $state.go('homepage.add-funny');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-funny',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.funnyTableParams.page(1);
            $scope.funnyTableParams.reload();
        };
        ...
        $scope.funnyTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
        ...
        //将\n替换成<br>
        $scope.displayAlias = function (alias) {
