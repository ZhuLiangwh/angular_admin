define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'pcBookmarkpushListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.pcbookmarkpush', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', pcBookmarkpushListCtrl]);
            return mod;
        }
    };

    function pcBookmarkpushListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/pcbookmarkpush/check_unique');
        }
        ...
        $scope.addBookmarkpush = function () {
            $state.go('pcpushservice.add-pcbookmarkpush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pcpushservice.edit-pcbookmarkpush',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.pcbookmarkpushTableParams.page(1);
            $scope.pcbookmarkpushTableParams.reload();
        };
        ...
        $scope.pcbookmarkpushTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
