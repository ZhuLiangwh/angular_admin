define([], function () {
    var diName = 'pcBookmarkpushEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state','$modal', '$window', '$location', 'ds.pcbookmarkpush', '$log', 'dpDialog',pcBookmarkpushEditCtrl]);
            return mod;
        }
    };

    function pcBookmarkpushEditCtrl($rootScope, $scope,apiService, $state,$modal, $window, $location, DS, $log, dpDialog) {
        ...
        //提交时对应三种不同的数据
        $scope.add = {method:'add', id: stateParams.id,can_delete:true, order:1};
        $scope.modify = {method:'modify', id: stateParams.id,can_delete:true, order:1};
        $scope.delete = {method:'delete', id: stateParams.id,can_delete:true, order:1};
        ...
        $scope.popUp = function(method, moduleName, modelName,label,index,params,single){
            ...
            dpDialog.loadIframe({
                ...
                from:'pcpushservice_bookmarkpush'
            });
        };
        ...
        $scope.popUp_pcbookmark = function(method, moduleName, modelName,label,index,params,single){
            $scope.popUp(method, moduleName, modelName,label,index,params,single);
            curRefItem = 'pcbookmark_modify';
        };
