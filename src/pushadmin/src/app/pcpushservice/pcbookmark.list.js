define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    ...
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.pcbookmark', 'logger', 'apiService', 'PER_PAGE','dpDialog', pcBookmarkListCtrl]);
            return mod;
        }
    };
    ...
    function pcBookmarkListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addBookmark = function () {
            $state.go('pcpushservice.add-pcbookmark');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pcpushservice.edit-pcbookmark', {
                id: item.id
            });
        };
    ...
    $scope.filter = function(node) {
      ...
      $scope.bookmarkTableParams.page(1);
      $scope.bookmarkTableParams.reload();
    };
        ...
        $scope.bookmarkTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
