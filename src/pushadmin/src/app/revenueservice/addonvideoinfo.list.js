define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AddonvideoinfoListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.addonvideoinfo', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', AddonvideoinfoListCtrl]);
            return mod;
        }
    };

    function AddonvideoinfoListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/addonvideoinfo/check_unique');
        }
        ...
        $scope.addAddonvideoinfo = function () {
            $state.go('revenueservice.add-addonvideoinfo');
        };

        $scope.ViewDetail = function (item) {
            $state.go('revenueservice.edit-addonvideoinfo',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.addonvideoinfoTableParams.page(1);
            $scope.addonvideoinfoTableParams.reload();
        };
        ...
        $scope.addonvideoinfoTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
