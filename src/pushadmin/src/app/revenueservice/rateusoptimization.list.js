define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'RateusoptimizationListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.rateusoptimization', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', RateusoptimizationListCtrl]);
            return mod;
        }
    };

    function RateusoptimizationListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addRateusoptimization = function () {
            $state.go('revenueservice.add-rateusoptimization');
        };

        $scope.ViewDetail = function (item) {
            $state.go('revenueservice.edit-rateusoptimization',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.rateusoptimizationTableParams.page(1);
            $scope.rateusoptimizationTableParams.reload();
        };
        ...
        $scope.rateusoptimizationTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
