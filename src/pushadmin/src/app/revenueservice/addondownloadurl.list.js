define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AddondownloadurlListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.addondownloadurl', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', AddondownloadurlListCtrl]);
            return mod;
        }
    };

    function AddondownloadurlListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/addondownloadurl/check_unique');
        }
        ...
        $scope.addAddondownloadurl = function () {
            $state.go('revenueservice.add-addondownloadurl');
        };

        $scope.ViewDetail = function (item) {
            $state.go('revenueservice.edit-addondownloadurl',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.addondownloadurlTableParams.page(1);
            $scope.addondownloadurlTableParams.reload();
        };
        ...
        $scope.addondownloadurlTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
