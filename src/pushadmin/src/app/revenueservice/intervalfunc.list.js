define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'IntervalfuncListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.intervalfunc', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', IntervalfuncListCtrl]);
            return mod;
        }
    };

    function IntervalfuncListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        //     return apiService.getApiUrl('/intervalfunc/check_unique');
        // }
        ...
        $scope.addIntervalfunc = function () {
            $state.go('revenueservice.add-intervalfunc');
        };

        $scope.ViewDetail = function (item) {
            $state.go('revenueservice.edit-intervalfunc',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.intervalfuncTableParams.page(1);
            $scope.intervalfuncTableParams.reload();
        };
        ...
        $scope.intervalfuncTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
