define(['text!layouts/confirm-dialog.html'], function (confirmDialogTpl) {
    var diName = 'AoslocaleListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aoslocale', 'logger', 'apiService', 'PER_PAGE', AoslocaleListCtrl]);
            return mod;
        }
    };

    function AoslocaleListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE) {
        var apiParams = {};
        var vm = $scope.vm = {
            'listChecked': [],
            'listTotal': 0
        ...
        $scope.addAoslocale = function () {
            $state.go('aosrule.add-aoslocale');
        };

        $scope.ViewDetail = function (item) {
            $state.go('aosrule.edit-aoslocale', {
                id: item.id
            });
        };
        ...
        $scope.goSearch = function () {
            ...
            $scope.aoslocaleTableParams.page(1);
            $scope.aoslocaleTableParams.reload();
        };


        $scope.aoslocaleTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
