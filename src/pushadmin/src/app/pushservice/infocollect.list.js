define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'InfocollectListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.infocollect', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', InfocollectListCtrl]);
            return mod;
        }
    };

    function InfocollectListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/infocollect/check_unique');
        }
        ...
        $scope.addInfocollect = function () {
            $state.go('pushservice.add-infocollect');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-infocollect',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.infocollectTableParams.page(1);
            $scope.infocollectTableParams.reload();
        };
        ...
        $scope.infocollectTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
