define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'SearcherpushListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.searcherpush', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', SearcherpushListCtrl]);
            return mod;
        }
    };

    function SearcherpushListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/searcherpush/check_unique');
        }
        ...
        $scope.addSearcherpush = function () {
            $state.go('pushservice.add-searcherpush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-searcherpush',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.searcherpushTableParams.page(1);
            $scope.searcherpushTableParams.reload();
        };
        ...
        $scope.searcherpushTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
