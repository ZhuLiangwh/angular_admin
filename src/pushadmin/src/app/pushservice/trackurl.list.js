define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'TrackurlListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.trackurl', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', TrackurlListCtrl]);
            return mod;
        }
    };

    function TrackurlListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/trackurl/check_unique');
        }
        ...
        $scope.addTrackurl = function () {
            $state.go('pushservice.add-trackurl');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-trackurl',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.trackurlTableParams.page(1);
            $scope.trackurlTableParams.reload();
        };
        ...
        $scope.trackurlTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
