define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AddonmessageListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.addonmessage', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', AddonmessageListCtrl]);
            return mod;
        }
    };

    function AddonmessageListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.addAddonmessage = function () {
            $state.go('pushservice.add-addonmessage');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-addonmessage',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.addonmessageTableParams.page(1);
            $scope.addonmessageTableParams.reload();
        };
        ...
        $scope.addonmessageTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
