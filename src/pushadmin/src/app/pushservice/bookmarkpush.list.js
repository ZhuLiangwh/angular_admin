define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'BookmarkpushListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.bookmarkpush', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', BookmarkpushListCtrl]);
            return mod;
        }
    };

    function BookmarkpushListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/bookmarkpush/check_unique');
        }
        ...
        $scope.addBookmarkpush = function () {
            $state.go('pushservice.add-bookmarkpush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-bookmarkpush',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.bookmarkpushTableParams.page(1);
            $scope.bookmarkpushTableParams.reload();
        };
        ...
        $scope.bookmarkpushTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
