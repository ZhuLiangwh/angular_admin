define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'UrlredirectListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter','$location', '$modal', 'ngTableParams', 
                'ds.urlredirect', 'logger', 'apiService','dpErrorHandler','dpDialog', 'PER_PAGE', UrlredirectListCtrl]);
            return mod;
        }
    };

    function UrlredirectListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,dpDialog,PER_PAGE) {
        ...
        $scope.addUrlredirect = function () {
            $state.go('pushservice.add-urlredirect');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-urlredirect',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.urlredirectTableParams.page(1);
            $scope.urlredirectTableParams.reload();
        };
        ...
        $scope.urlredirectTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
