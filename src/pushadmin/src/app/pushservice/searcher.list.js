define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'SearcherListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.searcher', 'logger', 'apiService', 'PER_PAGE','dpDialog', SearcherListCtrl]);
            return mod;
        }
    };

    function SearcherListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addSearcher = function () {
            $state.go('pushservice.add-searcher');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-searcher', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.searcherTableParams.page(1);
            $scope.searcherTableParams.reload();
        };
        ...
        $scope.searcherTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
