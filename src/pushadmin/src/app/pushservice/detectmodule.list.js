define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'DetectmoduleListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.detectmodule', 'logger', 'apiService','dpErrorHandler','dpDialog' , 'PER_PAGE', DetectmoduleListCtrl]);
            return mod;
        }
    };

    function DetectmoduleListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,dpDialog,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/detectmodule/check_unique');
        }
        ...
        $scope.addDetectmodule = function () {
            $state.go('pushservice.add-detectmodule');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-detectmodule',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.detectmoduleTableParams.page(1);
            $scope.detectmoduleTableParams.reload();
        };
        ...
        $scope.detectmoduleTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
