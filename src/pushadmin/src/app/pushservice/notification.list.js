define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'NotificationListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.notification', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', NotificationListCtrl]);
            return mod;
        }
    };

    function NotificationListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addNotification = function () {
            $state.go('pushservice.add-notification');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-notification',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.notificationTableParams.page(1);
            $scope.notificationTableParams.reload();
        };
        ...
        $scope.notificationTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
