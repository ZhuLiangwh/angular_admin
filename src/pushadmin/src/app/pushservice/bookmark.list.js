define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'BookmarkListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.bookmark', 'logger', 'apiService', 'PER_PAGE','dpDialog', BookmarkListCtrl]);
            return mod;
        }
    };

    function BookmarkListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addBookmark = function () {
            $state.go('pushservice.add-bookmark');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-bookmark', {
                id: item.id
            });
        };
