define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'EngineswitchListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.engineswitch', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', EngineswitchListCtrl]);
            return mod;
        }
    };

    function EngineswitchListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/engineswitch/check_unique');
        }
        ...
        $scope.addEngineswitch = function () {
            $state.go('pushservice.add-engineswitch');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-engineswitch',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.engineswitchTableParams.page(1);
            $scope.engineswitchTableParams.reload();
        };
        ...
        $scope.engineswitchTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
