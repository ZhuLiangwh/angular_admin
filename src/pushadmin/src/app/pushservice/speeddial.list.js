define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'SpeeddialListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.speeddial', 'logger', 'apiService', 'PER_PAGE','dpDialog', SpeeddialListCtrl]);
            return mod;
        }
    };

    function SpeeddialListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addSpeeddial = function () {
            $state.go('pushservice.add-speeddial');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-speeddial', {
                id: item.id
            });
        };
