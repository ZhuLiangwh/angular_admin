define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'RateusListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.rateus', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', RateusListCtrl]);
            return mod;
        }
    };

    function RateusListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addRateus = function () {
            $state.go('pushservice.add-rateus');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-rateus', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.rateusTableParams.page(1);
            $scope.rateusTableParams.reload();
        };
        ...
        $scope.rateusTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
