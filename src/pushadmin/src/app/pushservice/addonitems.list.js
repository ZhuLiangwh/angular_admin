define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AddonitemsListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.addonitems', 'logger', 'apiService', 'PER_PAGE','dpDialog', AddonitemsListCtrl]);
            return mod;
        }
    };

    function AddonitemsListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addAddonitems = function () {
            $state.go('pushservice.add-addonitems');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-addonitems', {
                id: item.id
            });
        };
        ...
        $scope.filter = function(node) {
            ...
            $scope.addonitemsTableParams.page(1);
            $scope.addonitemsTableParams.reload();
        };
        ...
        $scope.addonitemsTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
