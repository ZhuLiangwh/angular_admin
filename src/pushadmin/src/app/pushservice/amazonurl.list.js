define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AmazonurlListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.amazonurl', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', AmazonurlListCtrl]);
            return mod;
        }
    };

    function AmazonurlListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/amazonurl/check_unique');
        }
        ...
        $scope.addAmazonurl = function () {
            $state.go('pushservice.add-amazonurl');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-amazonurl',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.amazonurlTableParams.page(1);
            $scope.amazonurlTableParams.reload();
        };
        ...
        $scope.amazonurlTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
