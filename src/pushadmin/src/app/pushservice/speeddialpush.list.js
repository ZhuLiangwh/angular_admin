define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'SpeeddialpushListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.speeddialpush', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', SpeeddialpushListCtrl]);
            return mod;
        }
    };

    function SpeeddialpushListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/speeddialpush/check_unique');
        }
        ...
        $scope.addSpeeddialpush = function () {
            $state.go('pushservice.add-speeddialpush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-speeddialpush',{id:item.id});
        };
