define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'UserExperienceSwitchListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.userExperienceSwitch', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', UserExperienceSwitchListCtrl]);
            return mod;
        }
    };

    function UserExperienceSwitchListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/userExperienceSwitch/check_unique');
        }
        ...
        $scope.addUserExperienceSwitch = function () {
            $state.go('pushservice.add-userExperienceSwitch');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-userExperienceSwitch',{id:item.id});
        };
        ...
        $scope.filter = function (node, isInit) {
            if (!isInit) {
                ...
                $scope.userexpeTableParams.page(1);
                $scope.userexpeTableParams.reload();
            }
        };
    
        $scope.goSearch = function () {
            apiParams.searchKeyword = $scope.search.string;
            $scope.userExperienceSwitchTableParams.page(1);
            $scope.userExperienceSwitchTableParams.reload();
        };

        $scope.userExperienceSwitchTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
