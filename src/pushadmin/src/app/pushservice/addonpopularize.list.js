define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AddonpopularizeListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.addonpopularize', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', AddonpopularizeListCtrl]);
            return mod;
        }
    };

    function AddonpopularizeListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/addonpopularize/check_unique');
        }
        ...
        $scope.addAddonpopularize = function () {
            $state.go('pushservice.add-addonpopularize');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-addonpopularize',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.addonpopularizeTableParams.page(1);
            $scope.addonpopularizeTableParams.reload();
        };
        ...
        $scope.addonpopularizeTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
