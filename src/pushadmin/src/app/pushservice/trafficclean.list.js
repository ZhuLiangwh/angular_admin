define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'TrafficcleanListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.trafficclean', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', TrafficcleanListCtrl]);
            return mod;
        }
    };

    function TrafficcleanListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addTrafficclean = function () {
            $state.go('pushservice.add-trafficclean');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-trafficclean',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.trafficcleanTableParams.page(1);
            $scope.trafficcleanTableParams.reload();
        };
        ...
        $scope.trafficcleanTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
