define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'UrlitemListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.urlitem', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', UrlitemListCtrl]);
            return mod;
        }
    };

    function UrlitemListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/urlitem/check_unique');
        }
        ...
        $scope.addUrlitem = function () {
            $state.go('pushservice.add-urlitem');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-urlitem',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.urlitemTableParams.page(1);
            $scope.urlitemTableParams.reload();
        };
        ...
        $scope.urlitemTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
