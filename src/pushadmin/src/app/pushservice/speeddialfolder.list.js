define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'SpeeddialfolderListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.speeddialfolder', 'logger', 'apiService', 'PER_PAGE','dpDialog', SpeeddialfolderListCtrl]);
            return mod;
        }
    };

    function SpeeddialfolderListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addSpeeddialfolder = function () {
            $state.go('pushservice.add-speeddialfolder');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-speeddialfolder', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.speeddialfolderTableParams.page(1);
            $scope.speeddialfolderTableParams.reload();
        };
        ...
        $scope.speeddialfolderTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
