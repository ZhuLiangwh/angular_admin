define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'EnginestatusListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.enginestatus', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', EnginestatusListCtrl]);
            return mod;
        }
    };

    function EnginestatusListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE, dpDialog) {
        ...
        $scope.addEnginestatus = function () {
            $state.go('pushservice.add-enginestatus');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-enginestatus',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.enginestatusTableParams.page(1);
            $scope.enginestatusTableParams.reload();
        };
        ...
        $scope.enginestatusTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
