define(['common/utils/date'], function (dateUtil) {
    var diName = 'TrackurlEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state','$modal', '$window', '$location', 'ds.trackurl', '$log', 'dpDialog',TrackurlEditCtrl]);
            return mod;
        }
    };

    function TrackurlEditCtrl($rootScope, $scope,apiService, $state,$modal, $window, $location, DS, $log, dpDialog) {
        ...
        //select acitce
        $scope.action_type = [
            {value:'开启Track URL',name:'开启Track URL'},
            {value:'关闭Track URL',name:'关闭Track URL'}
        ];
