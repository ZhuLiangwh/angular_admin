define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'BookmarkfolderListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.bookmarkfolder', 'logger', 'apiService', 'PER_PAGE','dpDialog', BookmarkfolderListCtrl]);
            return mod;
        }
    };

    function BookmarkfolderListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addBookmarkfolder = function () {
            $state.go('pushservice.add-bookmarkfolder');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-bookmarkfolder', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.bookmarkfolderTableParams.page(1);
            $scope.bookmarkfolderTableParams.reload();
        };
        ...
        $scope.bookmarkfolderTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
