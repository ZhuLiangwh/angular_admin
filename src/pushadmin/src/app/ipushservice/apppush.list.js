/**
 * Created by liangzhu on 2015/5/26.
 */
define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'ApppushListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.iapppush', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', ApppushListCtrl]);
            return mod;
        }
    };

    function ApppushListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addApppush = function () {
            $state.go('ipushservice.add-iapppush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-iapppush',{id:item.id});
        };
        ...
        $scope.getDownUrl = function(){
            return vm.listChecked.length === 0 ? 'javascript:;' : apiService.getApiUrl('/iapppush/export?id=' + vm.listChecked.join(','));
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.iapppushTableParams.page(1);
            $scope.iapppushTableParams.reload();
        };
        ...
        $scope.iapppushTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
