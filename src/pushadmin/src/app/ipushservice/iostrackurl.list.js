define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'IostrackurlListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.iostrackurl', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', IostrackurlListCtrl]);
            return mod;
        }
    };

    function IostrackurlListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/iostrackurl/check_unique');
        }
        ...
        $scope.addIostrackurl = function () {
            $state.go('ipushservice.add-iostrackurl');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-iostrackurl',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.iostrackurlTableParams.page(1);
            $scope.iostrackurlTableParams.reload();
        };
        ...
        $scope.iostrackurlTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
