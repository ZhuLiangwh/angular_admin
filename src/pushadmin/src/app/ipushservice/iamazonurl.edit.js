define(['common/utils/date'], function (dateUtil) {
    var diName = 'iAmazonurlEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state','$modal', '$window', '$location', 'ds.iamazonurl', '$log', 'dpDialog',iAmazonurlEditCtrl]);
            return mod;
        }
    };

    function iAmazonurlEditCtrl($rootScope, $scope,apiService, $state,$modal, $window, $location, DS, $log, dpDialog) {
        ...
        $scope.entity.forceaddall =false;
        ...
        //select acitce
        $scope.action_type = [
            {value:'添加URL',name:'添加URL'},
            {value:'删除URL',name:'删除URL'}
        ];
        ...
        $scope.afterVali = function(){
            ...
            if(!valiOrder([$scope.entity.iurlitem])){
                alert('order不能为空');
                return false;
            }
        }
