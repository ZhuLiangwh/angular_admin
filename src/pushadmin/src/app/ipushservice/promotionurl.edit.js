define(['common/utils/date'], function (dateUtil) {
    var diName = 'iPromotionurlEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state','$modal', '$window', '$location', 'ds.ipromotionurl', '$log', 'dpDialog',iPromotionurlEditCtrl]);
            return mod;
        }
    };

    function iPromotionurlEditCtrl($rootScope, $scope,apiService, $state,$modal, $window, $location, DS, $log, dpDialog) {
        ...
        $scope.popUp = function(method, moduleName, modelName,label,index,params,single){
            ...
            dpDialog.loadIframe({
                ...
                from:'promotionservice_ipromotionurl'
            });
        };
