define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'iBookmarkfolderListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.ibookmarkfolder', 'logger', 'apiService', 'PER_PAGE','dpDialog', iBookmarkfolderListCtrl]);
            return mod;
        }
    };

    function iBookmarkfolderListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addiBookmarkfolder = function () {
            $state.go('ipushservice.add-ibookmarkfolder');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-ibookmarkfolder', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.ibookmarkfolderTableParams.page(1);
            $scope.ibookmarkfolderTableParams.reload();
        };
        ...
        $scope.ibookmarkfolderTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
