define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'iUrlparamListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.iurlparam', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', iUrlparamListCtrl]);
            return mod;
        }
    };

    function iUrlparamListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.addiUrlparam = function () {
            $state.go('ipushservice.add-iurlparam');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-iurlparam',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.iurlparamTableParams.page(1);
            $scope.iurlparamTableParams.reload();
        };
        ...
        $scope.iurlparamTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
