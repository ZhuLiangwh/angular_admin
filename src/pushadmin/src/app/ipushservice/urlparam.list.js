define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'UrlparamListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.urlparam', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', UrlparamListCtrl]);
            return mod;
        }
    };

    function UrlparamListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/urlparam/check_unique');
        }
        ...
        $scope.addUrlparam = function () {
            $state.go('pushservice.add-urlparam');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pushservice.edit-urlparam',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.urlparamTableParams.page(1);
            $scope.urlparamTableParams.reload();
        };
        ...
        $scope.urlparamTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
