define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'iAmazonurlListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.iamazonurl', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', iAmazonurlListCtrl]);
            return mod;
        }
    };

    function iAmazonurlListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/iamazonurl/check_unique');
        }
        ...
        $scope.addiAmazonurl = function () {
            $state.go('ipushservice.add-iamazonurl');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-iamazonurl',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.iamazonurlTableParams.page(1);
            $scope.iamazonurlTableParams.reload();
        };
        ...
        $scope.iamazonurlTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
