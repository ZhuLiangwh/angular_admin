define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'iSearcherpushListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.isearcherpush', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', iSearcherpushListCtrl]);
            return mod;
        }
    };

    function iSearcherpushListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/isearcherpush/check_unique');
        }
        ...
        $scope.addiSearcherpush = function () {
            $state.go('ipushservice.add-isearcherpush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-isearcherpush',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.isearcherpushTableParams.page(1);
            $scope.isearcherpushTableParams.reload();
        };
        ...
        $scope.isearcherpushTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
