define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'iPromotionurlListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.ipromotionurl', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', iPromotionurlListCtrl]);
            return mod;
        }
    };

    function iPromotionurlListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/ilinkpush/check_unique');
        }
        ...
        $scope.addiPromotionurl = function () {
            $state.go('ipushservice.add-ilinkpush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-ilinkpush',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.ipromotionurlTableParams.page(1);
            $scope.ipromotionurlTableParams.reload();
        };
        ...
        $scope.ipromotionurlTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
