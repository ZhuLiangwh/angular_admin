define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'iBookmarkpushListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.ibookmarkpush', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', iBookmarkpushListCtrl]);
            return mod;
        }
    };

    function iBookmarkpushListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/ibookmarkpush/check_unique');
        }
        ...
        $scope.addBookmarkpush = function () {
            $state.go('ipushservice.add-ibookmarkpush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-ibookmarkpush',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.ibookmarkpushTableParams.page(1);
            $scope.ibookmarkpushTableParams.reload();
        };
        ...
        $scope.ibookmarkpushTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
