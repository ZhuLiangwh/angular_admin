define([], function () {
    var diName = 'iSpeeddialpushEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state','$modal', '$window', '$location', 'ds.ispeeddialpush', '$log', 'dpDialog',iSpeeddialpushEditCtrl]);
            return mod;
        }
    };

    function iSpeeddialpushEditCtrl($rootScope, $scope,apiService, $state,$modal, $window, $location, DS, $log, dpDialog) {
        ...
        //提交时对应三种不同的数据
        $scope.add = {method:'add', id: stateParams.id,can_delete:true};
        $scope.modify = {method:'modify', id: stateParams.id,can_delete:true};
        $scope.delete = {method:'delete', id: stateParams.id,can_delete:true};
        ...
        $scope.popUp = function(method, moduleName, modelName,label,index,params,single){
            ...
            dpDialog.loadIframe({
                ...
                from:'ipushservice_speeddialpush'
            });
        };
        ...
        $scope.popUp_ispeeddial = function(method, moduleName, modelName,label,index,params,single){
            $scope.popUp(method, moduleName, modelName,label,index,params,single);
            curRefItem = 'ispeeddial_modify';
        };
