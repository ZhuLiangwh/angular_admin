define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'iSpeeddialfolderListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.ispeeddialfolder', 'logger', 'apiService', 'PER_PAGE','dpDialog', iSpeeddialfolderListCtrl]);
            return mod;
        }
    };

    function iSpeeddialfolderListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addSpeeddialfolder = function () {
            $state.go('ipushservice.add-ispeeddialfolder');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-ispeeddialfolder', {
                id: item.id
            });
        };
