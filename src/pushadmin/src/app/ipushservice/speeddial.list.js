define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'iSpeeddialListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.ispeeddial', 'logger', 'apiService', 'PER_PAGE','dpDialog', iSpeeddialListCtrl]);
            return mod;
        }
    };

    function iSpeeddialListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addSpeeddial = function () {
            $state.go('ipushservice.add-ispeeddial');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-ispeeddial', {
                id: item.id
            });
        };
    ...
    $scope.filter = function(node) {
      ...
      $scope.speeddialTableParams.page(1);
      $scope.speeddialTableParams.reload();
    };
        ...
        $scope.speeddialTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
