define([], function () {
    var diName = 'iSpeeddialfolderEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state', '$window', '$location', 'ds.ispeeddialfolder', '$log','dpDialog', iSpeeddialfolderEditCtrl]);
            return mod;
        }
    };

    function iSpeeddialfolderEditCtrl($rootScope, $scope,apiService, $state, $window, $location, DS, $log,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/ispeeddialfolder/check_unique');
        }
        ...
        $scope.popUp = function(method, moduleName, modelName,label,index,params,single){
            ...
            dpDialog.loadIframe({
                ...
                from:'ipushservice_speeddialfolder'
            });
        };
