define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'iSpeeddialpushListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.ispeeddialpush', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', iSpeeddialpushListCtrl]);
            return mod;
        }
    };

    function iSpeeddialpushListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/ispeeddialpush/check_unique');
        }
        ...
        $scope.addSpeeddialpush = function () {
            $state.go('ipushservice.add-ispeeddialpush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-ispeeddialpush',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.speeddialpushTableParams.page(1);
            $scope.speeddialpushTableParams.reload();
        };
        ...
        $scope.speeddialpushTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
