define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'UrloptionfolderListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.iurloptionfolder', 'logger', 'apiService', 'PER_PAGE','dpDialog', UrloptionfolderListCtrl]);
            return mod;
        }
    };

    function UrloptionfolderListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addUrloptionfolder = function () {
            $state.go('ipushservice.add-iurloptionfolder');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-iurloptionfolder', {
                id: item.id
            });
        };
        ...
        $scope.filter = function(node) {
            ...
            $scope.iurloptionfolderTableParams.page(1);
            $scope.iurloptionfolderTableParams.reload();
        };
        ...
        $scope.iurloptionfolderTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
