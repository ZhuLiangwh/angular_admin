define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'UrloptionListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.iurloption', 'logger', 'apiService', 'PER_PAGE','dpDialog', UrloptionListCtrl]);
            return mod;
        }
    };

    function UrloptionListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addUrloption = function () {
            $state.go('ipushservice.add-iurloption');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-iurloption', {
                id: item.id
            });
        };
        ...
        $scope.filter = function(node) {
            ...
            $scope.iurloptionTableParams.page(1);
            $scope.iurloptionTableParams.reload();
        };
        ...
        $scope.iurloptionTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
