define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'iBookmarkListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.ibookmark', 'logger', 'apiService', 'PER_PAGE','dpDialog', iBookmarkListCtrl]);
            return mod;
        }
    };

    function iBookmarkListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addBookmark = function () {
            $state.go('ipushservice.add-ibookmark');
        };

        $scope.ViewDetail = function (item) {
            $state.go('ipushservice.edit-ibookmark', {
                id: item.id
            });
        };
