define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'PromotionurlListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.promotionurl', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', PromotionurlListCtrl]);
            return mod;
        }
    };

    function PromotionurlListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/promotionurl/check_unique');
        }
        ...
        $scope.addPromotionurl = function () {
            $state.go('promotionservice.add-promotionurl');
        };

        $scope.ViewDetail = function (item) {
            $state.go('promotionservice.edit-promotionurl',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.promotionurlTableParams.page(1);
            $scope.promotionurlTableParams.reload();
        };
        ...
        $scope.promotionurlTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
