define(['common/utils/date'], function (dateUtil) {
    var diName = 'PromotionspeeddialEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state','$modal', '$window', '$location', 'ds.promotionspeeddial', '$log', 'dpDialog',PromotionspeeddialEditCtrl]);
            return mod;
        }
    };

    function PromotionspeeddialEditCtrl($rootScope, $scope,apiService, $state,$modal, $window, $location, DS, $log, dpDialog) {
        ...
        $scope.popUp = function(method, moduleName, modelName,label,index,params,single){
            ...
            dpDialog.loadIframe({
                ...
                from:'promotionservice_promotionspeeddial'
            });
        };
