define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/alert-dialog.html', 'text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl, alertDialogTpl) {
    var diName = 'ApkListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.apk', 'logger', 'apiService', 'dpErrorHandler', 'PER_PAGE','dpDialog',ApkListCtrl]);
            return mod;
        }
    };

    function ApkListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler, PER_PAGE,dpDialog) {
        var apiParams = {};
        var vm = $scope.vm = {
            'listChecked': [],
            'listTotal': 0
        ...
        $scope.addApk = function () {
            $state.go('staticresource.add-apk');
        };

        $scope.ViewDetail = function (item) {
            $state.go('staticresource.edit-apk', {id: item.id});
        };
        ...
        $scope.filter = function (node, isInit) {
            if (!isInit) {
                ...
                $scope.ApkTableParams.page(1);
                $scope.ApkTableParams.reload();
            }
        };
        ...
        $scope.ApkTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
