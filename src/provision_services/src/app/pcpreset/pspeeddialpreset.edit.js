define([], function () {
    var diName = 'PspeeddialpresetEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state','$modal', '$window', '$location', 'ds.pspeeddialpreset', '$log', 'dpDialog',PspeeddialpresetEditCtrl]);
            return mod;
        }
    };

    function PspeeddialpresetEditCtrl($rootScope, $scope,apiService, $state,$modal, $window, $location, DS, $log, dpDialog) {
        ...
        $scope.popUp = function(method, moduleName, modelName,label,index,params,single){
            ...
            dpDialog.loadIframe({
                ...
                from:'aospreset_pspeeddialpreset'
            });
        };
