define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'PCbookmarkfolderListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.pbookmarkfolder', 'logger', 'apiService', 'PER_PAGE','dpDialog', PCbookmarkfolderListCtrl]);
            return mod;
        }
    };

    function PCbookmarkfolderListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addPCbookmarkfolder = function () {
            $state.go('pcpreset.add-pbookmarkfolder');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pcpreset.edit-pbookmarkfolder', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.pbookmarkfolderTableParams.page(1);
            $scope.pbookmarkfolderTableParams.reload();
        };
        ...
        $scope.pbookmarkfolderTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
