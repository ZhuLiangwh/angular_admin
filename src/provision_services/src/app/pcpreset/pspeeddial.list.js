define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'PspeeddialListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.pspeeddial', 'logger', 'apiService', 'PER_PAGE','dpDialog', PspeeddialListCtrl]);
            return mod;
        }
    };

    function PspeeddialListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addPspeeddial = function () {
            $state.go('pcpreset.add-pspeeddial');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pcpreset.edit-pspeeddial', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.pspeeddialTableParams.page(1);
            $scope.pspeeddialTableParams.reload();
        };
        ...
        $scope.pspeeddialTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
