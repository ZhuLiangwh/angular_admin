define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'PspeeddialpresetListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.pspeeddialpreset', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', PspeeddialpresetListCtrl]);
            return mod;
        }
    };

    function PspeeddialpresetListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/pspeeddialpreset/check_unique');
        }
        ...
        $scope.addPspeeddialpreset = function () {
            $state.go('pcpreset.add-pspeeddialpreset');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pcpreset.edit-pspeeddialpreset',{id:item.id});
        };
        ...
        /*$scope.export = function(){
            if (vm.listChecked.length === 0) return;
            DS.export(vm.listChecked).then(function(data){
                logger.success('export success.');
            },function(data){
                logger.error('export faild.');
            });
        };*/

        $scope.getDownUrl = function(){
            return vm.listChecked.length === 0 ? 'javascript:;' : apiService.getApiUrl('/pspeeddialpreset/export?id=' + vm.listChecked.join(','));
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.pspeeddialpresetTableParams.page(1);
            $scope.pspeeddialpresetTableParams.reload();
        };
        ...
        $scope.pspeeddialpresetTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
