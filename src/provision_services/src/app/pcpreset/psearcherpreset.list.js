define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'PsearcherpresetListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.psearcherpreset', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', PsearcherpresetListCtrl]);
            return mod;
        }
    };

    function PsearcherpresetListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/psearcherpreset/check_unique');
        }
        ...
        $scope.addPsearcherpreset = function () {
            $state.go('pcpreset.add-psearcherpreset');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pcpreset.edit-psearcherpreset',{id:item.id});
        };
        ...
        $scope.getDownUrl = function(){
            return vm.listChecked.length === 0 ? 'javascript:;' : apiService.getApiUrl('/psearcherpreset/export?id=' + vm.listChecked.join(','));
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.psearcherpresetTableParams.page(1);
            $scope.psearcherpresetTableParams.reload();
        };
        ...
        $scope.psearcherpresetTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
