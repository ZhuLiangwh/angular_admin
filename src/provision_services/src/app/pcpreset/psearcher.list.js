define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'PsearcherListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.psearcher', 'logger', 'apiService', 'PER_PAGE','dpDialog', PsearcherListCtrl]);
            return mod;
        }
    };

    function PsearcherListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addPsearcher = function () {
            $state.go('pcpreset.add-psearcher');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pcpreset.edit-psearcher', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.psearcherTableParams.page(1);
            $scope.psearcherTableParams.reload();
        };
        ...
        $scope.psearcherTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
