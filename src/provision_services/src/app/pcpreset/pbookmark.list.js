define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'PCbookmarkListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.pbookmark', 'logger', 'apiService', 'PER_PAGE','dpDialog', PCbookmarkListCtrl]);
            return mod;
        }
    };

    function PCbookmarkListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addPCbookmark = function () {
            $state.go('pcpreset.add-pbookmark');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pcpreset.edit-pbookmark', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.pbookmarkTableParams.page(1);
            $scope.pbookmarkTableParams.reload();
        };
        ...
        $scope.pbookmarkTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
