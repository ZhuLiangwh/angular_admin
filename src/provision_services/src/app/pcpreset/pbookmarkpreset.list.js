define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'PbookmarkpresetListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.pbookmarkpreset', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', PbookmarkpresetListCtrl]);
            return mod;
        }
    };

    function PbookmarkpresetListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addPbookmarkpreset = function () {
            $state.go('pcpreset.add-pbookmarkpreset');
        };

        $scope.ViewDetail = function (item) {
            $state.go('pcpreset.edit-pbookmarkpreset',{id:item.id});
        };
        ...
        $scope.getDownUrl = function(){
            return vm.listChecked.length === 0 ? 'javascript:;' : apiService.getApiUrl('/pbookmarkpreset/export?id=' + vm.listChecked.join(','));
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.pbookmarkpresetTableParams.page(1);
            $scope.pbookmarkpresetTableParams.reload();
        };
        ...
        $scope.pbookmarkpresetTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
