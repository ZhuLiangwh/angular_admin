define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'AosspeeddialdesktopListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aosspeeddialdesktop', 'logger', 'apiService', 'PER_PAGE','dpDialog', AosspeeddialdesktopListCtrl]);
            return mod;
        }
    };

    function AosspeeddialdesktopListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addAosspeeddialdesktop = function () {
            $state.go('aospreset.add-aosspeeddialdesktop');
        };

        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aosspeeddialdesktop', {
                id: item.id
            });
        };
        ...
        $scope.delete = function(){
            ...
            del('您确定要删除吗',{items:vm.listChecked},delSuccess,function(error){
                if(error.data.status === 1005){
                    del('您删除的数据被初始数据引用，确定要删除吗?',{items:vm.listChecked,comfirm:true},delSuccess,function(error){
        ...
        $scope.filter = function (node) {
            ...
            $scope.aosspeeddialdesktopTableParams.page(1);
            $scope.aosspeeddialdesktopTableParams.reload();
        };
        ...
        $scope.aosspeeddialdesktopTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
