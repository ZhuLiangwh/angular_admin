define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'AosrecommendshareListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aosrecommendshare', 'logger', 'apiService', 'PER_PAGE','dpDialog', AosrecommendshareListCtrl]);
            return mod;
        }
    };
    function AosrecommendshareListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addAosrecommendshare = function () {
            $state.go('aospreset.add-aosrecommendshare');
        };
        ...
        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aosrecommendshare', {
                id: item.id
            });
        };
        ...
        $scope.delete = function () {
            ...
            del('您确定要删除吗', {items: vm.listChecked}, delSuccess, function (error) {
                if (error.data.status === 1005) {
                    del('您删除的数据被share引用，确定要删除吗?', {items: vm.listChecked, comfirm: true}, delSuccess, function (error) {
        ...
        $scope.filter = function (node) {
            ...
            $scope.aosrecommendshareTableParams.page(1);
            $scope.aosrecommendshareTableParams.reload();
        };
        ...
        $scope.aosrecommendshareTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
