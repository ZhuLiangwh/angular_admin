define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'AossearcherfolderListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aossearcherfolder', 'logger', 'apiService', 'PER_PAGE','dpDialog', AossearcherfolderListCtrl]);
            return mod;
        }
    };

    function AossearcherfolderListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addAossearcherfolder = function () {
            $state.go('aospreset.add-aossearcherfolder');
        };

        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aossearcherfolder', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.aossearcherfolderTableParams.page(1);
            $scope.aossearcherfolderTableParams.reload();
        };
        ...
        $scope.aossearcherfolderTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
