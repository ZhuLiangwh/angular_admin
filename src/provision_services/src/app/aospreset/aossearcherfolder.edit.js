define([], function () {
    var diName = 'AossearcherfolderEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state', '$window', '$location', 'ds.aossearcherfolder', '$log','dpDialog', AossearcherfolderEditCtrl]);
            return mod;
        }
    };

    function AossearcherfolderEditCtrl($rootScope, $scope,apiService, $state, $window, $location, DS, $log,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/aossearcherfolder/check_unique');
        }
        ...
        $scope.popUp = function(method, moduleName, modelName,label,index,params,single){
            ...
            dpDialog.loadIframe({
                ...
                from:'aospreset_aossearcherfolder'
            });
        };
            ...
            for (var key in $scope.entity) {
                ...
                if (_.isArray(item) && item.length > 0 && _.isObject(item[0])) {
                    $scope.entity[key] = _.filter(item, function (filterItem) {
                        if($scope.entity.defaultCheck === filterItem.id && filterItem.isDelete){$scope.entity.defaultCheck = ''}
