define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'AosspeeddialscreenListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aosspeeddialscreen', 'logger', 'apiService', 'PER_PAGE','dpDialog', AosspeeddialscreenListCtrl]);
            return mod;
        }
    };

    function AosspeeddialscreenListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addAosspeeddialscreen = function () {
            $state.go('aospreset.add-aosspeeddialscreen');
        };

        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aosspeeddialscreen', {
                id: item.id
            });
        };
        ...
        $scope.delete = function () {
            ...
            del('您确定要删除吗', {items: vm.listChecked}, delSuccess, function (error) {
                if (error.data.status === 1005) {
                    del('您删除的数据被桌面引用，确定要删除吗?', {items: vm.listChecked, comfirm: true}, delSuccess, function (error) {
        ...
        $scope.filter = function (node) {
            ...
            $scope.aosspeeddialscreenTableParams.page(1);
            $scope.aosspeeddialscreenTableParams.reload();
        };
        ...
        $scope.aosspeeddialscreenTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
