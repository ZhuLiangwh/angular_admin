define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'AosspeeddialfolderListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aosspeeddialfolder', 'logger', 'apiService', 'PER_PAGE','dpDialog', AosspeeddialfolderListCtrl]);
            return mod;
        }
    };

    function AosspeeddialfolderListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addAosspeeddialfolder = function () {
            $state.go('aospreset.add-aosspeeddialfolder');
        };

        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aosspeeddialfolder', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.aosspeeddialfolderTableParams.page(1);
            $scope.aosspeeddialfolderTableParams.reload();
        };
        ...
        $scope.aosspeeddialfolderTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
