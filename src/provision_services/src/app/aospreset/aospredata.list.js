define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AospredataListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aospredata', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', AospredataListCtrl]);
            return mod;
        }
    };

    function AospredataListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/aospredata/check_unique');
        }
        ...
        $scope.addAospredata = function () {
            $state.go('aospreset.add-aospredata');
        };

        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aospredata',{id:item.id});
        };
        ...
        $scope.getDownUrl = function(){
            return vm.listChecked.length === 0 ? 'javascript:;' : apiService.getApiUrl('/aospredata/export?id=' + vm.listChecked.join(','));
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.aospredataTableParams.page(1);
            $scope.aospredataTableParams.reload();
        };
        ...
        $scope.aospredataTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
