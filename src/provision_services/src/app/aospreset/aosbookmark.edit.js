define([], function () {
    var diName = 'AosbookmarkEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', 'apiService','$scope', '$state', '$window', '$location', 'ds.aosbookmark', '$log','dpDialog', AosbookmarkEditCtrl]);
            return mod;
        }
    };

    function AosbookmarkEditCtrl($rootScope, apiService,$scope, $state, $window, $location, DS, $log,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/aosbookmark/check_unique');
        }
