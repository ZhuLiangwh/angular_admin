define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'AostemplateshareListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aostemplateshare', 'logger', 'apiService', 'PER_PAGE','dpDialog', AostemplateshareListCtrl]);
            return mod;
        }
    };
    function AostemplateshareListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addAostemplateshare = function () {
            $state.go('aospreset.add-aostemplateshare');
        };
        ...
        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aostemplateshare', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.aostemplateshareTableParams.page(1);
            $scope.aostemplateshareTableParams.reload();
        };
        ...
        $scope.aostemplateshareTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
