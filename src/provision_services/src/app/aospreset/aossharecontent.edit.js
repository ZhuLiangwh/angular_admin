define([], function () {
    var diName = 'AossharecontentEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope', '$state', 'apiService','$window', '$location', 'ds.aossharecontent', '$log','dpDialog', AossharecontentEditCtrl]);
            return mod;
        }
    };

    function AossharecontentEditCtrl($rootScope, $scope, $state,apiService, $window, $location, DS, $log, dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/aossharecontent/check_unique');
        }
        ...
        $scope.popUp = function(method, moduleName, modelName,label,index,params,single){
            ...
            dpDialog.loadIframe({
                ...
                from:'aospreset_aossharecontent'
            });
        };
