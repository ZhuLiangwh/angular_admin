define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'AossharecontentListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aossharecontent', 'logger', 'apiService', 'PER_PAGE','dpDialog', AossharecontentListCtrl]);
            return mod;
        }
    };

    function AossharecontentListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addAossharecontent = function () {
            $state.go('aospreset.add-aossharecontent');
        };
        ...
        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aossharecontent', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.aossharecontentTableParams.page(1);
            $scope.aossharecontentTableParams.reload();
        };
        ...
        $scope.aossharecontentTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
