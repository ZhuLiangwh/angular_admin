define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'AossearcherListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aossearcher', 'logger', 'apiService', 'PER_PAGE','dpDialog', AossearcherListCtrl]);
            return mod;
        }
    };

    function AossearcherListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addAossearcher = function () {
            $state.go('aospreset.add-aossearcher');
        };

        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aossearcher', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.aossearcherTableParams.page(1);
            $scope.aossearcherTableParams.reload();
        };
        ...
        $scope.aossearcherTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
