define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'AosspeeddialListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aosspeeddial', 'logger', 'apiService', 'PER_PAGE','dpDialog', AosspeeddialListCtrl]);
            return mod;
        }
    };

    function AosspeeddialListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addAosspeeddial = function () {
            $state.go('aospreset.add-aosspeeddial');
        };

        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aosspeeddial', {
                id: item.id
            });
        };
    ...
    $scope.filter = function(node) {
      ...
      $scope.aosspeeddialTableParams.page(1);
      $scope.aosspeeddialTableParams.reload();
    };
        ...
        $scope.aosspeeddialTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
