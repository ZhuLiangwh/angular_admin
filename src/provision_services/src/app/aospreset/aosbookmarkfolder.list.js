define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl) {
    var diName = 'AosbookmarkfolderListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aosbookmarkfolder', 'logger', 'apiService', 'PER_PAGE','dpDialog', AosbookmarkfolderListCtrl]);
            return mod;
        }
    };

    function AosbookmarkfolderListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog) {
        ...
        $scope.addAosbookmarkfolder = function () {
            $state.go('aospreset.add-aosbookmarkfolder');
        };

        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aosbookmarkfolder', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.aosbookmarkfolderTableParams.page(1);
            $scope.aosbookmarkfolderTableParams.reload();
        };
        ...
        $scope.aosbookmarkfolderTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
