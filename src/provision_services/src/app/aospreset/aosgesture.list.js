define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html', 'text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl, alertDialogTpl) {
    var diName = 'AosgestureListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.aosgesture', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE','dpDialog', AosgestureListCtrl]);
            return mod;
        }
    };

    function AosgestureListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler, PER_PAGE,dpDialog) {
        ...
        $scope.addAosgesture = function () {
            $state.go('aospreset.add-aosgesture');
        };

        $scope.ViewDetail = function (item) {
            $state.go('aospreset.edit-aosgesture', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.aosgestureTableParams.page(1);
            $scope.aosgestureTableParams.reload();
        };
        ...
        $scope.aosgestureTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
