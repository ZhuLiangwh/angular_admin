define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'HotwordListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.hotword', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', HotwordListCtrl]);
            return mod;
        }
    };

    function HotwordListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addhotWord = function () {
            $state.go('homepage.add-hotWord');
        };

        $scope.ViewDetail = function (item) {
            $state.go('homepage.edit-hotWord',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.hotwordTableParams.page(1);
            $scope.hotwordTableParams.reload();
        };
        ...
        $scope.hotwordTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
