define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'AdblockerlocaleListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.Adblockerlocale', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', AdblockerlocaleListCtrl]);
            return mod;
        }
    };

    function AdblockerlocaleListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addAdblocker = function () {
            $state.go('adblocker.add-adblockerlocale');
        };

        $scope.ViewDetail = function (item) {
            $state.go('adblocker.edit-adblockerlocale', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.AdblockerlocaleTableParams.page(1);
            $scope.AdblockerlocaleTableParams.reload();
        };
        ...
        $scope.AdblockerlocaleTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
