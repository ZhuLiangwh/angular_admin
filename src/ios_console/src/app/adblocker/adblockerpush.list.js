define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'AdblockerpushListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.adblockerpush', 'logger', 'apiService','dpErrorHandler', 'PER_PAGE', AdblockerpushListCtrl]);
            return mod;
        }
    };

    function AdblockerpushListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,PER_PAGE) {
        ...
        $scope.addadblockerpush = function () {
            $state.go('adblocker.add-adblockerpush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('adblocker.edit-adblockerpush',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.adblockerpushTableParams.page(1);
            $scope.adblockerpushTableParams.reload();
        };
        ...
        $scope.adblockerpushTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
