define([], function () {
    var diName = 'AdblockerlocaleEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state', '$window', '$location', 'ds.adblockerlocale', '$log','dpDialog', AdblockerlocaleEditCtrl]);
            return mod;
        }
    };

    function AdblockerlocaleEditCtrl($rootScope, $scope,apiService, $state, $window, $location, DS, $log,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/adblockerlocale/check_unique');
        }
        ...
        $scope.popUp = function(method, moduleName, modelName,label,index,params,single){
            ...
            dpDialog.loadIframe({
                ...
                from:'adblocker_adblockerlocale'
            });
        };
