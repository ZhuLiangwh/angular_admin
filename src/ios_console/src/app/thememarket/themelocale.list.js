define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'ThemelocaleListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.Themelocale', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', ThemelocaleListCtrl]);
            return mod;
        }
    };

    function ThemelocaleListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addTheme = function () {
            $state.go('thememarket.add-themelocale');
        };

        $scope.ViewDetail = function (item) {
            $state.go('thememarket.edit-themelocale', {
                id: item.id
            });
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.ThemelocaleTableParams.page(1);
            $scope.ThemelocaleTableParams.reload();
        };
        ...
        $scope.ThemelocaleTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
