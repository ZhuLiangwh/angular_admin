define(['common/utils/date', 'common/utils/dataConverter','text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html','common/utils/url'], function (dateUtil, dataConverter,confirmDialogTpl,alertDialogTpl,url) {
    var diName = 'ThemepushListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter','$location', '$modal', 'ngTableParams', 'ds.themepush', 'logger', 'apiService','dpErrorHandler','dpDialog', 'PER_PAGE', ThemepushListCtrl]);
            return mod;
        }
    };

    function ThemepushListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, dpErrorHandler,dpDialog,PER_PAGE) {
        ...
        $scope.addThemepush = function () {
            $state.go('thememarket.add-themepush');
        };

        $scope.ViewDetail = function (item) {
            $state.go('thememarket.edit-themepush',{id:item.id});
        };
        ...
        $scope.filter = function (node) {
            ...
            $scope.themepushTableParams.page(1);
            $scope.themepushTableParams.reload();
        };
        ...
        $scope.themepushTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
