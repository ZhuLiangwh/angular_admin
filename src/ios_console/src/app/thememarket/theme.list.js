define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'ThemeListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.theme', 'logger', 'apiService', 'PER_PAGE','dpDialog','dpErrorHandler', ThemeListCtrl]);
            return mod;
        }
    };

    function ThemeListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpDialog,dpErrorHandler) {
        ...
        $scope.addTheme = function () {
            $state.go('thememarket.add-theme');
        };

        $scope.ViewDetail = function (item) {
            $state.go('thememarket.edit-theme', {
                id: item.id
            });
        };
       ...
       function deleteItems(data){
        ...
        $scope.filter = function (node) {
            ...
            $scope.themeTableParams.page(1);
            $scope.themeTableParams.reload();
        };
        ...
        $scope.themeTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
