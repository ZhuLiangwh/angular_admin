define([], function () {
    var diName = 'ThemefolderEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state', '$window', '$location', 'ds.themefolder', '$log','dpDialog', ThemefolderEditCtrl]);
            return mod;
        }
    };

    function ThemefolderEditCtrl($rootScope, $scope,apiService, $state, $window, $location, DS, $log,dpDialog) {
        ...
        $scope.afterVali = function(){
            ...
            if(!valiOrder([$scope.entity.theme])){
                alert('order不能为空');
                return false;
            }
        }
