define([], function () {
    var diName = 'ThemelocaleEditCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$rootScope', '$scope','apiService', '$state', '$window', '$location', 'ds.Themelocale', '$log','dpDialog', ThemelocaleEditCtrl]);
            return mod;
        }
    };

    function ThemelocaleEditCtrl($rootScope, $scope,apiService, $state, $window, $location, DS, $log,dpDialog) {
        ...
        $scope.getCheckSortUrl = function() {
            return apiService.getApiUrl('/Themelocale/check_unique');
        }
        ...
        $scope.popUp = function(method, moduleName, modelName,label,index,params,single){
            ...
            dpDialog.loadIframe({
                ...
                from:'Themelocalemarket_Themelocale'
            });
        };
