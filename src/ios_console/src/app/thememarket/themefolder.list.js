define(['common/utils/date', 'common/utils/dataConverter', 'text!layouts/confirm-dialog.html','text!layouts/alert-dialog.html'], function (dateUtil, dataConverter, confirmDialogTpl,alertDialogTpl) {
    var diName = 'ThemefolderListCtrl';
    return {
        __register__: function (mod) {
            mod.controller(diName, ['$scope', '$window', '$state', '$filter', '$location', '$modal', 'ngTableParams', 'ds.themefolder', 'logger', 'apiService', 'PER_PAGE','dpErrorHandler','dpDialog', ThemefolderListCtrl]);
            return mod;
        }
    };

    function ThemefolderListCtrl($scope, $window, $state, $filter, $location, $modal, ngTableParams, DS, logger, apiService, PER_PAGE,dpErrorHandler,dpDialog) {
        ...
        $scope.addThemefolder = function () {
            $state.go('thememarket.add-themefolder');
        };

        $scope.ViewDetail = function (item) {
            $state.go('thememarket.edit-themefolder', {
                id: item.id
            });
        };
        ...
        $scope.delete = function () {
            ...
            del('您确定要删除吗', {items: vm.listChecked}, delSuccess, function (error) {
                if (error.data.status === 1005) {
                    del('您删除的数据被引用，确定要删除吗?', {items: vm.listChecked, comfirm: true}, delSuccess, function (error) {
        ...
        $scope.filter = function (node) {
            ...
            $scope.themefolderTableParams.page(1);
            $scope.themefolderTableParams.reload();
        };
        ...
        $scope.themefolderTableParams = new ngTableParams({
            page: 1,
            count: PER_PAGE
        }, {
