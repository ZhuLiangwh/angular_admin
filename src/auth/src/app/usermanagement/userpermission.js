...
define([],function(){
    ...
    return {
        __register__:function(mod){
            mod.controller(diName,['$scope','$stateParams','$window', 'ds.userlist', UserpermissionCtrl]);
            return mod;
        }
    };

    function UserpermissionCtrl($scope, $stateParams, $window, listDS){
        $scope.resource = [];
        $scope.projects = [];
        $scope.current = null;
        $scope.perms = [];
        ...
        $scope.toggleProject = function (index){
            ...
            if(!$scope.perms[index]){
                //获取内容
                listDS.getPermDisplayData({'project':$scope.projects[index],'uid':$stateParams.userid}).then(function(data){
                    var perm = $scope.perms[index] = data.data.data;

                    //选中状态的数据
                    listDS.getUserPermissions({'project':$scope.projects[index],'uid':$stateParams.userid}).then(function(data){
        ...
        $scope.submit = function(){
            ...
            listDS.saveUserPermissions({uid:$stateParams.userid,project:pro,perm:perm}).then(function(data){
                $window.history.back();
            },function(error){
